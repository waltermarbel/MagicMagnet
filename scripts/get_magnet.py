...
class GetMagnet:
	...
	def get_download_pages_from_test(search_url = None, result_url = None, startswith = None, not_in = None, slice = None):
		print(search_url)
		...
		for i in result.find_all("a", href = True):
			if i.get("href").startswith(startswith) and i.get("href") not in download_pages_links:
				valid = True
				
				if (startswith != None) and (not_in != None):
					for k in not_in:
						if k in i.get("href"):
							valid = False
